...
class Cursor(object):
    ...
    def insert(self, table, keys_list, values):
        """
        ...
        success = 0
        keys = ', '.join(keys_list)
        str_values = ', '.join(['%s'] * len(keys_list))
    ...
    def get_insert_sql(self, table, keys_list):
        keys = ', '.join(keys_list)
        str_values = ', '.join(['%s'] * len(keys_list))
        sql = "INSERT INTO {table}({keys}) VALUES ({values})".format(table=table, keys=keys, values=str_values)
        return sql

    def update(self, table, keys_list, values):
        """
        参数：table:str, keys:列表(内容必须为字符串), values:列表中套元祖
        return:0：表示失败；1：表示成功
        """
